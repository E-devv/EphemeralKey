...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="Ejemplos: python ephemeralkey.py --password -l 18 -s -c | python ephemeralkey.py --email --copy"
    )
    ...
    parser.add_argument('-c', '--copy', action='store_true', help='Copiar el resultado al portapapeles.')
    ...
    try:
        ...
        if args.copy:
            if not pyperclip:
                print("\n⚠️  Error: La librería 'pyperclip' no está instalada. No se pudo copiar al portapapeles.", file=sys.stderr)
                print("   Para instalarla, ejecuta: pip install pyperclip", file=sys.stderr)
                sys.exit(1)
            ...
            try:
                ...
                print(f"\n❌ Error: No se pudo acceder al portapapeles del sistema: {e}", file=sys.stderr)
                sys.exit(1)
